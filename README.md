...
* **Meeting Management & Review:**
    * View a list of past processed meetings, showing attendees, start time, and platform logo (Zoom, Google Meet, Microsoft Teams).
...
Both HubSpot and Salesforce share a **unified CRM modal component** (`CrmModalComponent`) parameterized by a `:crm` assign. This eliminates duplication and makes it straightforward to add more CRMs in the future — just add a new entry to the `@crm_modules` config map and a matching `@crm_config` entry.

### Shared CRM Modal Flow

1. **Contact Search:** Debounced input triggers CRM API search, results displayed in a styled dropdown
2. **AI Suggestions:** After selecting a contact, the app calls `HubspotSuggestions` or `SalesforceSuggestions` (which use Gemini) to generate field update suggestions from the meeting transcript
3. **Suggestion Cards:** Each card shows field label, current value (strikethrough), arrow, and suggested new value with a timestamp link to the transcript
4. **Selective Updates:** Checkbox per field allows selective updates; submit button disabled until at least one field is selected
5. **Apply:** Batch-updates selected contact properties via the respective CRM API

### HubSpot OAuth

* **Custom Ueberauth Strategy:** `lib/ueberauth/strategy/hubspot.ex`
* **OAuth 2.0 Flow:** Authorization code flow with HubSpot's `/oauth/authorize` and `/oauth/v1/token` endpoints
* **Token Refresh:** `HubspotTokenRefresher` Oban cron worker (every 5 min) proactively refreshes tokens nearing expiration

### Salesforce OAuth

* **Custom Ueberauth Strategy:** `lib/ueberauth/strategy/salesforce.ex` with **PKCE** support
* **OAuth 2.0 Flow:** Authorization code flow with PKCE for added security
* **Token Refresh:** `SalesforceTokenRefresher` Oban cron worker (every 5 min) proactively refreshes tokens nearing expiration
...
|-------|---------|------------|
| OAuth Strategy | `Ueberauth.Strategy.Hubspot` | `Ueberauth.Strategy.Salesforce` |
| API Behaviour | `SocialScribe.HubspotApiBehaviour` | `SocialScribe.SalesforceApiBehaviour` |
| Suggestions | `SocialScribe.HubspotSuggestions` | `SocialScribe.SalesforceSuggestions` |
| Modal Component | `CrmModalComponent` (`:crm` = `:hubspot`) | `CrmModalComponent` (`:crm` = `:salesforce`) |
| Parent Handlers | `MeetingLive.Show` — `@crm_modules` map | Same shared handlers |
...
The app is deployed on a VPS using Docker Compose with three services:

* **app** — Elixir release built via multi-stage Dockerfile (Elixir 1.18.3 / OTP 27.3.4)
* **db** — PostgreSQL 16
* **caddy** — Caddy 2 reverse proxy with automatic HTTPS

CI/CD is handled via GitHub Actions:
* **CI** (`.github/workflows/ci.yml`) — Runs `mix format --check-formatted`, `mix compile --warnings-as-errors`, and `mix test` on every PR to `master`
* **CD** (`.github/workflows/cd.yml`) — On push to `master`, SSHes into the VPS, pulls latest code, rebuilds, and runs migrations

### Running Tests

```bash
mix test                                    # Run all tests (324 tests + 30 property-based tests)
mix test test/path/to/file_test.exs         # Run a single test file
mix test test/path/to/file_test.exs:42      # Run a specific test at line
```
