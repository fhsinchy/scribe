defmodule SocialScribe.SalesforceApiPropertyTest do
  ...
  describe "apply_updates/3 properties" do
    ...
    property "returns {:ok, :no_updates} when all updates have apply: false", %{
      ...
      check all(updates <- list_of(update_generator(apply: false), min_length: 1, max_length: 10)) do
        result =
          SocialScribe.SalesforceApi.apply_updates(credential, "003XXXXXXXXXXXXXXX", updates)
      ...
      check all(updates <- list_of(update_generator(), min_length: 1, max_length: 10)) do
        ...
        updates_map =
          ...
          |> Enum.reduce(%{}, fn update, acc ->
            sf_field = Map.get(@field_mapping, update.field, update.field)
        ...
        applied_fields =
          updates
          |> Enum.filter(& &1[:apply])
          |> Enum.map(fn u -> Map.get(@field_mapping, u.field, u.field) end)
          |> MapSet.new()
      ...
      check all(updates <- list_of(update_generator(), min_length: 1, max_length: 10)) do
        ...
        for {field, value} <- updates_map do
          matching_updates =
            updates
            |> Enum.filter(fn u ->
              u[:apply] && Map.get(@field_mapping, u.field, u.field) == field
            end)
  ...
  describe "field_to_salesforce mapping properties" do
    property "all known fields map to PascalCase Salesforce field names" do
      check all(field <- member_of(@salesforce_fields)) do
        expected = Map.fetch!(@field_mapping, field)
        # Verify the mapping is consistent by checking the module attribute
        assert is_binary(expected)
        assert expected =~ ~r/^[A-Z]/
      end
    end

    property "field mapping is deterministic" do
      check all(field <- member_of(@salesforce_fields)) do
        first = Map.get(@field_mapping, field, field)
        second = Map.get(@field_mapping, field, field)
        assert first == second
      end
    end
  end
